...
**Про битые ссылки.** Битой считается ссылка, которой действительно нет: ответ
404 или 410 либо несуществующий домен. Отказы вида 403, 418, 429 и 5xx в эту
категорию не попадают — так отвечает защита от роботов, а в браузере ссылка
открывается нормально. Яндекс.Карты отдают 403 любому не-браузеру, ВКонтакте —
418 на HEAD-запрос. Такие адреса выносятся в отдельную информационную заметку
«не удалось проверить» и на оценку не влияют. Ссылки, отвергающие HEAD,
перепроверяются полноценным GET.
